local Net, this = LuaNetworking, {
    ...
    setMenuFunc = function( menu, func_data )
        ...
        if menu and type( func_data ) == "table" then
            func_data._ = func_data._ or function() end
...
if not AchieHunter then
    ...
    function AchieHunter:saveConfig()
        this.fileWrapper( self._save, "w+", nil, function( f )
            f:write( json.encode( self._conf ) )
        end)
    end

    function AchieHunter:loadConfig()
        this.fileWrapper( self._save, "r", nil, function( f )
    ...
    function AchieHunter:setupHooks( hook )
        if hook == "lib/setups/setup" then
            Hooks:PostHook( Setup, "update", this.hook( "SetupUpdate" ), function()
                if ( managers.hud and managers.hud._chat_focus ) or ( managers.menu_component and managers.menu_component._game_chat_gui and managers.menu_component._game_chat_gui:input_focus() ) then
                    return
                end

                local key, id = self:getVar( "key", "" ), self:getVar( "current", "" )
                if key ~= "" and os.clock() >= this.cooldown then
                    if key:find( "mouse " ) and Input:mouse():pressed( key:sub( 7 ) ) or Input:keyboard():pressed( key:id() ) then
        ...
        elseif hook == "lib/network/matchmaking/networkmatchmakingsteam" then
            Hooks:PostHook( NetworkMatchMakingSTEAM, "set_attributes", this.hook( "MatchmakingSetAttributes" ), function( menu )
        ...
        elseif hook == "lib/managers/crimenetmanager" then
                ...
                if button == Idstring( "1" ) then
                    for _, job in pairs( self._jobs ) do
                        if job.achievement and job.mouse_over == 1 then
                            QuickMenu:new(
                                job.host_name,
                                managers.localization:text( this.dialog.popup, { title = this.getAchievementInfo( job.achievement ) } ),
                                {}, true
                            )
            ...
            Hooks:PostHook( CrimeNetGui, "update_server_job", this.hook( "CrimeNetUpdateJob" ), function( menu, data, id )
                id = data.id or id
        ...
        elseif hook == "lib/managers/menu/achievementlistgui" then
            ...
            Hooks:PostHook( AchievementListGui, "keep_filling_list", this.hook( "AchievementGuiFill" ), function( menu )
            ...
            function AchievementListItem:_selected_changed(state)
                ...
                if self._id == this.active then
                    ...
                    self._desc:set_color(state and this.highlight_color:with_alpha(0.85) or this.highlight_color:with_alpha(0.5))
                else
            ...
            Hooks:PostHook( AchievementListGui, "init", this.hook( "AchievementGuiInit" ), function( menu )
            ...
            Hooks:PostHook( AchievementListGui, "close", this.hook( "AchievementGuiClose" ), function( menu )
                MenuCallbackHandler[this.name]()
            end)
        else
            Hooks:Add( "BaseNetworkSessionOnPeerEnteredLobby", this.hook( "SessionPeerEntered" ), function( peer, id )
                ...
                if data and Net:IsHost() then
                    ...
                    DelayedCalls:Add( this.name .. tostring( id ), 1, function()
            ...
            Hooks:Add( "BaseNetworkSessionOnEnteredLobby", this.hook( "SessionEnteredLobby" ), function()
                local data = this.getAchievementInfo( self:getVar( "current" ) )
                if data and Net:IsHost() then
            ...
            Hooks:Add( "CustomizeControllerOnKeySet", this.hook( "ControllerKeySet" ), function( keybind, key )
            ...
            Hooks:Add( "LocalizationManagerPostInit", this.hook( "LocalizationInit" ), function( manager )
            ...
            Hooks:Add( "MenuManagerSetupCustomMenus", this.hook( "MenuSetupMenus" ), function()
                ...
                MenuCallbackHandler[this.name] = function( menu, item )
                    if not item then
						if self:getVar( "current" ) ~= this.active then
                            ...
                            if this.active then
                                self:sendMessage( managers.localization:text( this.text.current, { title = this.getAchievementInfo( this.active ) } ), "" )
                            end
						end
            ...
            Hooks:Add( "MenuManagerPopulateCustomMenus", this.hook( "MenuPopulateMenus" ), function()
                local setup_data = {
            ...
            Hooks:Add( "MenuManagerBuildCustomMenus", this.hook( "MenuBuildMenus" ), function( _, nodes )
